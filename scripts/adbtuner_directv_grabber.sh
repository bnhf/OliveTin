...
# mapping (see stationId in createDirecTVJSON), harvested from other
# captures over time. Shared with ah4c_directv_grabber.sh - lives alongside
# the script (not /config) so it is tracked in this repo; in the container
# the script itself runs from /config, so scriptDir resolves there
# automatically at runtime. sdCallSignPairs is for the rarer case where an
# HD/SD pair does not even resolve as a pair automatically (neither
# nameBase nor csBase matches) - a manually confirmed SD callSign -> HD
# callSign mapping.
...
createDirecTVJSON() {
  adbtunerDirecTVJSON=$(
    ...
    | jq --arg uuid "$customConfigUUID" --arg scheme "$urlScheme" --argjson stationIdOverrides "$stationIdOverrides" --argjson sdCallSignPairs "$sdCallSignPairs" '
      ...
      # first and, when it finds a match, is used exclusively - callSign
      # alone is not reliable enough, since two different shows can share
      # one raw callSign (e.g. "Angels Broadcast Television" and "...Extra"
      # are both callSign "ABTV", only the channelName tells them apart).
      # csBase is only consulted as a fallback when no entry shares a
      # nameBase, which is the common case for local affiliates whose HD/SD
      # names diverge (e.g. "... A3 HD" vs "... SD") but whose callSign does
      # not (e.g. "KTLA").
      ...
      # DirecTV often has not back-filled a real Gracenote/TMS mapping onto
      # the HD row of a pair - externalListingId just echoes the row own
      # ccid instead. Treat that as "no real id".
      def isFakeId: (.externalListingId == .ccid);
      # Within a duplicate channelNumber group, drop a non-HD entry only
      # when an HD entry in the same group is its counterpart. Anything
      # left over (true alternates, e.g. "ION East" vs "WNBA on ION 1/2/3",
      # or distinct feeds like "TNT HD" vs "TNT West HD") falls through
      # unchanged to the suffix logic below. When the surviving HD entry
      # has no real station id, prefer a previously-harvested id for its
      # own callSign (see stationIdOverridesFile) over borrowing from its
      # SD counterpart in this same capture - the override is a known-good
      # id, whereas the SD counterpart id is merely "not obviously fake"
      # and could itself be stale or wrong.
      def collapseHDPairs:
        . as $g
      ...
      # For a duplicate channelNumber group left over after HD/SD
      # collapsing: the one entry that is neither "Alternate" nor ends in a
      # number is the main channel and keeps the bare number. Entries named
      # "... Alternate" get sequential .1/.2 suffixes; entries named
      # "... N" (e.g. "WNBA on ION 1") get a suffix matching their own
      # trailing number instead of their position in the source data. If
      # every entry in the group is itself numbered and none stands out as
      # a plain main (e.g. "ESPN+ 1" through "ESPN+ 7", with no bare
      # "ESPN+"), treat the lowest-numbered entry as the implicit main so
      # the rest still line up with their own number. Anything else that
      # does not resolve to exactly one main channel falls back to the
      # original order-based suffixing so nothing is silently mishandled.
      def suffixGroup:
        . as $g
...
main() {
  ...
  echo "$adbtunerDirecTVJSON" | createDirecTVChannels
  cdvrCustomSource
}
