...
runningScripts() {
  ...
  for server in "${servers[@]}"; do
    activeProcess=$(ps -ef | grep "[a]dbalerter.sh $server" | awk '{print $2}')
...
scriptKill() {
  ...
  rm /config/"$channelsHost"-"$channelsPort"_adbalerts.running
  echo "Killing E-Mail ADB Alerts PID $runningScriptPID"
  sleep 2
  lastActive=$(ps -e | grep adbalerter | awk '{print $1}')
  ...
  if [[ -z $lastActive ]]; then
    ...
    sed -i "/#${foregroundScript} container_name default/s/default: .* #/default: ah4c #/" $configTemp
    exit 0
  fi
...
scriptRun() {
  ...
  nohup /config/adbalerter.sh $dvr $frequency $ah4cContainer >> $logFile 2>&1 &
  runningScriptPID=$!
  echo "$foregroundScript.sh $dvr $frequency $ah4cContainer" > /config/"$channelsHost"-"$channelsPort"_adbalerts.running
