...
urlScheme="${1:-dtvnow://deeplink.directvnow.com/play/channel/}"
configurationUUID="$2"
...
# Shared with ah4c_directv_grabber.sh and adbtuner_directv_grabber.sh - lives
# alongside the script (not /config) so it is tracked in this repo; in the
# container the script itself runs from /config, so scriptDir resolves there
# automatically at runtime. sdCallSignPairs is for the rarer case where an
# HD/SD pair does not even resolve as a pair automatically (neither nameBase
# nor csBase matches, e.g. "American Heroes HD" callSign AHCHD vs "American
# Heroes Channel" callSign AHCH) - a manually confirmed SD callSign -> HD
# callSign mapping.
...
generateJSON() {
  ...
  | jq --arg urlScheme "$urlScheme" --arg configurationUUID "$configurationUUID" --argjson stationIdOverrides "$stationIdOverrides" --argjson sdCallSignPairs "$sdCallSignPairs" '
    ...
    [
      ...
      | {
          ...
          name: .channelName,
          url: ($urlScheme + .callSign + "/" + .resourceId),
    ...
    # sort_order is a 1-based running index over the final channel list,
    # emitted as a one-decimal string ("1.0", "2.0", ...) to match the
    # native-app station-record format.
    | to_entries
    | map(.value + { sort_order: ((.key + 1 | tostring) + ".0") })
  '
}
