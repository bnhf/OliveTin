...
channelwatchDNS="${45}"
  [[ -n $channelwatchDNS ]] && channelwatchDNSArgument="--dns $channelwatchDNS"
...
scriptRun() {
  ...
  [[ "$(docker inspect --format='{{ index .Config.Labels "version" }}' coderluii/channelwatch:latest)" != "$channelwatchVersion" ]] && docker pull coderluii/channelwatch:0.5
  nohup docker run --name $containerName $channelwatchDNSArgument --dns-search $olivetinSearch $optionalArguments -v $containerName:/config --restart unless-stopped coderluii/channelwatch:0.5 >> $logFile 2>&1 &
  runningScriptPID=$!
